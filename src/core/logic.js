...
// === ГЕНЕРАТОР ОТМАЗОК ПСИЧА ===
function getPsychErrorReply(errText) {
    ...
    if (error.includes('503') || error.includes('overloaded') || error.includes('unavailable') || error.includes('timeout')) {
        const phrases = [
            ...
            "💤 Чет мне впадлу отвечать, ебитесь сами, настроение будет - отвечу"
        ];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    ...
    const phrases = [
        "🛠 Так, у меня шестеренки встали. Какая-то дичь в коде",
        "💥 Я упал. Критическая ошибка.",
        "🚑 Хьюстон, у нас проблемы. Я поймал баг и не знаю, что делать."
    ];
    return phrases[Math.floor(Math.random() * phrases.length)];
}
...
async function processMessage(bot, msg) {
    ...
    if (!storage.hasChat(chatId) && !config.isAdmin(chatId)) {
        ...
        // Шлем всем админам тихонько
        config.adminIds.forEach(adminId => {
            bot.sendMessage(adminId, alertText, { parse_mode: 'Markdown' }).catch(() => { });
        });
    }
    ...
    if (msg.chat.type === 'private' && !config.isAdmin(userId)) {
...
Бот работает на моих API-ключах Google, и я отвечаю за всё, что он генерирует. Поэтому он работает только там, где есть я (в чатах) или в моей личке.`;
    ...
    // Проверка выхода админа обрабатывается в index.js
...
• Напомню сделать что угодно, чтобы ты не забыл, просто напиши «Псич напомни [Завтра, в следующий четверг или через пару минут]» — и я поставлю напоминание, или ответь реплаем на сообщение, в котором есть дата или время события.

**🎲 Развлекуха:**
• "Псич кинь монетку" — Орел/Решка.
• "Псич число 1-(цифра)" — Рандомное число.
• "Псич кто из нас (текст)" — Выберу виноватого или счастливчика из чата.

**🕵️ Досье и Память:**
• "Псич кто я?" — Мое честное мнение о тебе.
• "Псич расскажи про @юзера" — Выдам базу про участника.
...
• /reset — Сброс памяти (если я начал тупить или забыл контекст).
    ...
    if (command === '/mute') {
        if (!config.isAdmin(userId)) {
            return bot.sendMessage(chatId, "❌ Эта команда доступна только администратору.", getBaseOptions(threadId));
        }
    ...
    if (command === '/restart' && config.isAdmin(userId)) {
        await bot.sendMessage(chatId, "🔄 Перезагружаюсь...", getBaseOptions(threadId));
        exec('pm2 restart sych-bot', (err) => {
            if (err) {
                config.adminIds.forEach(adminId => {
                    bot.sendMessage(adminId, `❌ Ошибка рестарта: ${err.message}`).catch(() => { });
                });
            }
        });
        return;
    ...
    if (hasTriggerWord) {
        ...
        const isWhoGame = cleanText.match(/(?:кто|кого)\s+(?:из нас|тут|здесь|в чате|сегодня)/) || cleanText.match(/псич\W+кто\??$/) || cleanText.trim() === "псич кто";
        if (isWhoGame) {
            try { await bot.sendChatAction(chatId, 'typing', getActionOptions(threadId)); } catch (e) { }
    ...
    if (isDirectlyCalled) {
        ...
        if (text.length > 10 && Math.random() < 0.05) {
    ...
    if (shouldAnswer) {
        ...
        // Если сообщение очень короткое (только триггер), добавляем контекст
        if (text.trim().toLowerCase() === 'псич' || text.trim().toLowerCase() === 'psych') {
            text = 'Псич, привет! Что нужно?';
        }
        ...
        try {
            ...
            if (!aiResponse || aiResponse.trim().length === 0) {
                console.log(`[DEBUG] 🚨 ОШИБКА: AI вернул пустую строку! Исходный текст: "${text}"`);

                // Определяем причину пустого ответа
                let errorReason = "empty response";
                const lowerText = text.toLowerCase();

                // Проверяем, не был ли запрос слишком сложным/длинным
                if (text.length > 5000) {
                    errorReason = "400 too large";
                } else if (lowerText.includes('10000') || lowerText.includes('1000') || lowerText.includes('много символов')) {
                    // Запрос на очень длинный ответ
                    errorReason = "400 too large";
                }

                config.adminIds.forEach(adminId => {
                    bot.sendMessage(adminId, `⚠️ **ALARM:** Gemini вернула пустую строку!\n📂 **Чат:** ${chatTitle}\n💬 **Текст:** "${text.substring(0, 200)}${text.length > 200 ? '...' : ''}"\n🔍 **Причина:** ${errorReason}`, { parse_mode: 'Markdown' }).catch(() => { });
                });

                // Если сообщение было очень коротким (просто "Псич"), даем дефолтный ответ
                const originalText = msg.text || msg.caption || "";
                if (originalText.trim().toLowerCase() === 'псич' || originalText.trim().toLowerCase() === 'psych') {
                    aiResponse = "🐱 У аппарата. Что нужно?";
                } else {
                    // Генерируем более умный ответ в зависимости от причины
                    if (errorReason.includes('too large')) {
                        aiResponse = "Ты дофига просишь. Мне это нах не надо";
                    } else {
                        aiResponse = getPsychErrorReply("503 overloaded");
                    }
                }
            }
            ...
            // 1. ШЛЕМ ТЕХНИЧЕСКИЙ РЕПОРТ ВСЕМ АДМИНАМ (В личку)
            const errorMsg = `🔥 **Gemini упала!**\n\nЧат: ${chatTitle}\nОшибка: \`${err.message}\``;
            config.adminIds.forEach(adminId => {
                bot.sendMessage(adminId, errorMsg, { parse_mode: 'Markdown' }).catch(() => { });
            });

            // 2. ГЕНЕРИРУЕМ СМЕШНОЙ ОТВЕТ ДЛЯ ЧАТА
            // Передаем текст ошибки в нашу новую функцию
            aiResponse = getPsychErrorReply(err.message);
        }
        ...
        try {
            ...
            addToHistory(chatId, "Псич", aiResponse);
            ...
            // Отчет всем админам
            config.adminIds.forEach(adminId => {
                bot.sendMessage(adminId, `⚠️ **Ошибка отправки:** ${error.message}\n📂 **Чат:** ${chatTitle}\n🆔 **ID:** ${chatId}`, { parse_mode: 'Markdown' }).catch(() => { });
            });
